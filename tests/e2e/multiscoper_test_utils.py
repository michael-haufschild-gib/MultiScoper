"""
MultiScoper Test Harness Client

HTTP client for driving the MultiScoper test harness. All UI automation flows through
the C++ TestHttpServer running on localhost:8765. This module provides:

  - MultiScoperTestClient: stateless HTTP client for element queries, interactions,
...
class MultiScoperTestClient:
    """HTTP client for the MultiScoper test harness."""
    ...
    def open_editor(self, track_id: int = 0, wait_timeout_s: float = 10.0) -> None:
        ...
        # Poll for elements to appear. Assign the JSON response once per
        # poll iteration instead of fetching it twice (the previous
        # `lambda: _get_json(...) and _get_json(...)` pattern issued two
        # HTTP round-trips per check — wasted over the full e2e suite
        # since every test opens the editor).
        def elements_ready():
            resp = self._get_json("/ui/elements")
            return resp is not None and resp.get("data", {}).get("count", 0) > 0
    ...
    def click_at_offset(self, element_id: str, offset_x: int, offset_y: int) -> bool:
        """Click at a specific pixel offset within an element.

        Use when an interactive region is sub-bounds — e.g. the swatch
        strip on an oscillator list item that opens the color dialog,
        distinct from the settings button trigger at a different offset.
        """
        return self._post_ok(
            "/ui/clickAtOffset",
            {"elementId": element_id, "offsetX": offset_x, "offsetY": offset_y},
        )

    def double_click_at_offset(self, element_id: str, offset_x: int, offset_y: int) -> bool:
        return self._post_ok(
            "/ui/doubleClickAtOffset",
            {"elementId": element_id, "offsetX": offset_x, "offsetY": offset_y},
        )
