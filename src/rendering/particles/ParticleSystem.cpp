...
static const char* particleVertexShader = R"(
    ...
    void main()
    {
        ...
        if (uTurbulence.x > 0.001) {
            ...
            // Sample 3D noise
            // Coordinate: Position scaled + Time moving Z slice
            ...
            // Apply to position
            turbOffset = force * strength * 50.0; // Scale factor for visibility
        }
...
// Fragment shaders remain the same
...
bool ParticleSystem::initialize(juce::OpenGLContext& context)
{
    ...
    // 32x32x32 texture
    const int size = 32;
    std::vector<uint8_t> noiseData(size * size * size * 4); // RGBA8
    ...
    for (int z = 0; z < size; ++z)
    {
        for (int y = 0; y < size; ++y)
        {
            for (int x = 0; x < size; ++x)
            {
                ...
                // Generate 2 components for vector field (XY)
                float n1 = Noise3D::perlin(fx, fy, fz);
                float n2 = Noise3D::perlin(fx + 100.0f, fy + 100.0f, fz + 100.0f);

                size_t idx = static_cast<size_t>(((z * size + y) * size + x) * 4);
                noiseData[idx + 0] = static_cast<uint8_t>((n1 * 0.5f + 0.5f) * 255.0f);
                noiseData[idx + 1] = static_cast<uint8_t>((n2 * 0.5f + 0.5f) * 255.0f);
                noiseData[idx + 2] = 0;
                noiseData[idx + 3] = 255;
            }
        }
    ...
    glBindTexture(GL_TEXTURE_3D, noiseTextureID_);
    // Note: glTexImage3D requires OpenGL 1.2+ functions, handled by GLEW or JUCE extensions usually.
    // JUCE's extensions doesn't always expose glTexImage3D directly in the OpenGLExtension struct if not generated.
    // But standard macOS OpenGL 3.3+ supports it.
    // Using raw glTexImage3D.
...
bool ParticleSystem::compileShaders(juce::OpenGLContext& context)
{
    auto compile = [&](ShaderProgram& shader, const char* fragSource) -> bool {
        ...
        shader.noiseTextureLoc = shader.program->getUniformIDFromName("uNoiseTexture"); // New
        ...
        shader.turbulenceLoc = shader.program->getUniformIDFromName("uTurbulence"); // New
        shader.timeLoc = shader.program->getUniformIDFromName("uTime"); // New
        return true;
    };
...
void ParticleSystem::update(float deltaTime)
{
    ...
    // CPU update only handles basic physics (Gravity/Drag)
    // Turbulence is now handled in the Vertex Shader
                            ...
                            const ParticleSettings& settings)
{
    ...
    // Bind Noise Texture to Unit 2 (0=Sprite, 1=Depth)
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_3D, noiseTextureID_);
    ext.glUniform1i(shader->noiseTextureLoc, 2);

    // Set Turbulence Uniforms
    // Check if turbulence is enabled in settings
    // For now, use global system settings or pass them in?
    // The `render` method calls `settings`. But turbulence params were on `ParticleSystem` global.
    // We should use the global params for now.
    ext.glUniform3f(shader->turbulenceLoc, turbulenceStrength_, turbulenceScale_, turbulenceSpeed_);
    ext.glUniform1f(shader->timeLoc, time_);
