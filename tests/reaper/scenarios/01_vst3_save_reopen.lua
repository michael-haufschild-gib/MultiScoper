...
-- Task 3.2 — MultiScoper VST3 save/reopen round-trip.
--
...
--   * Reaper has already scanned MultiScoper.vst3 (see README prerequisites).
--   * TrackFX_AddByName accepts the "VST3:MultiScoper" prefix form.
--   * Main_SaveProjectEx writes synchronously on return (we verify via io.open).
--   * dump_plugin_state returns a non-empty string containing "MultiScoper" — both
...
local SAVE_PATH = "/tmp/multiscoper_reaper_scenario_01.rpp"

local function run()
  T.new_project()

  -- Load MultiScoper VST3 on track 0 (0-indexed).
  local fx = T.load_vst3("MultiScoper", 0)
  T.assert_true(fx >= 0, "VST3 MultiScoper load returned negative fx index")
  ...
  T.assert_true(T.state_mentions_plugin(before, "MultiScoper"),
    "pre-save chunk does not mention MultiScoper")
  ...
  T.assert_true(name:find("MultiScoper", 1, true) ~= nil,
    "FX at track 0 slot 0 is not MultiScoper after reopen (got '" .. name .. "')")
  ...
  T.assert_true(T.state_mentions_plugin(after, "MultiScoper"),
    "post-reopen chunk does not mention MultiScoper")
