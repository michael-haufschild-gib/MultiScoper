/*
    MultiScoper - Accordion Component
 ...
 */
class MultiScoperAccordionSection
...
public:
    ...
    explicit MultiScoperAccordionSection(IThemeService& themeService, juce::String title = "");
    /// Create accordion section with title and test ID.
    MultiScoperAccordionSection(IThemeService& themeService, juce::String title, const juce::String& testId);
    ~MultiScoperAccordionSection() override;
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(MultiScoperAccordionSection)
};
 ...
 * - Keyboard navigation
 */
class MultiScoperAccordion : public ThemedComponent
{
public:
    /// Create an accordion container.
    explicit MultiScoperAccordion(IThemeService& themeService);
    ~MultiScoperAccordion() override;

    /// Add a section with title only (content set later via section->setContent).
    MultiScoperAccordionSection* addSection(const juce::String& title);
    /// Add a section with title and content component.
    MultiScoperAccordionSection* addSection(const juce::String& title, juce::Component* content);
    ...
    MultiScoperAccordionSection* getSection(int index);
...
private:
    ...
    std::vector<std::unique_ptr<MultiScoperAccordionSection>> sections_;
    bool allowMultiExpand_ = false;
    int spacing_ = 1;

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(MultiScoperAccordion)
};
