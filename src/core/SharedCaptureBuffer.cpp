                                ...
                                bool tryLock)
{
    ...
    const float* channels[MAX_CHANNELS] = {nullptr}; // NOLINT(misc-const-correctness)
                                        ...
                                        const CaptureFrameMetadata& metadata)
{
    ...
    jassert(numChannels >= 0 && std::cmp_less_equal(numChannels, MAX_CHANNELS));
    if (numSamples <= 0)
        return;
    ...
    size_t const writePos = writePos_.load(std::memory_order_relaxed);
...
{
    size_t const written = samplesWritten_.load(std::memory_order_acquire);
