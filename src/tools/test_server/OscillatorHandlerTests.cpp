...
nlohmann::json runReorderTest(OscilState& state, OscilPluginEditor& editor,
                              const char* testName, int from, int to,
...
void OscillatorHandler::handleTestOscillatorReorder(const httplib::Request&, httplib::Response& res)
{
    auto result = runOnMessageThread([this]() -> nlohmann::json {
        ...
        state.reorderOscillators(2, 0); editor_.refreshPanels();

        tests.push_back(runReorderTest(state, editor_, "MoveLastToFirst", 2, 0, 0,
                                       "Last oscillator should move to first position"));
        state.reorderOscillators(0, 2); editor_.refreshPanels();

        tests.push_back(runReorderTest(state, editor_, "SwapAdjacent", 1, 0, 0,
        ...
        // Test 4: Verify orderIndex values are sequential
        {
            ...
            for (size_t i = 0; i < after.size(); ++i)
            {
                ...
                if (after[i].getOrderIndex() != static_cast<int>(i)) sequential = false;
            }
        ...
        // Test 5: Same index should be no-op
        {
            ...
            auto before = getOrderedOscillators(state);
            state.reorderOscillators(1, 1); editor_.refreshPanels();
            auto after = getOrderedOscillators(state);
            test["passed"] = (after[0].getName() == before[0].getName() &&
