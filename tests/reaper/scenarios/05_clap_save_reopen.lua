...
-- Task 3.6 — MultiScoper CLAP save/reopen round-trip.
--
...
local function try_add_clap(track_idx)
  ...
  -- Keep CLAP-specific identifiers only. The generic "MultiScoper" probe can
  ...
  local candidates = { "CLAP:MultiScoper", "CLAPi:MultiScoper", "clap.MultiScoper" }
...
local function run()
  ...
  T.assert_true(fx >= 0,
    "CLAP MultiScoper load failed for all prefix forms (CLAP:, CLAPi:, clap.). " ..
  ...
  T.assert_true(T.state_mentions_plugin(before, "MultiScoper"),
    "pre-save CLAP chunk does not mention MultiScoper")
  ...
  T.assert_true(name:find("MultiScoper", 1, true) ~= nil,
    "FX at track 0 slot 0 is not MultiScoper after CLAP reopen (got '" .. name .. "')")
  ...
  T.assert_true(T.state_mentions_plugin(after, "MultiScoper"),
    "post-reopen CLAP chunk does not mention MultiScoper")
