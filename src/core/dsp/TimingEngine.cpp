...
void TimingEngine::updateHostInfo(const juce::AudioPlayHead::PositionInfo& positionInfo)
{
    ...
    // Keep sync timestamp aligned on transport transitions when host sync
    // or restart-on-play behavior is active.
    ...
    // Restart-on-play: expose a one-shot trigger when transport starts.
    if (syncToPlayhead && !wasPlaying && isPlaying)
    {
        triggered_.store(true, std::memory_order_relaxed);
    }

    // Restart-on-playhead-discontinuity: trigger when timeline jumps backward
    // while transport remains in playing state (e.g., loop wrap or host seek).
    // Ignore tiny 1-2 sample host jitter to avoid noisy retriggers.
    if (syncToPlayhead && isPlaying && !playStateChanged && timeInSamples)
    {
        constexpr int64_t kBackwardJumpRetriggerMinSamples = 3;
        const int64_t backwardDeltaSamples = previousTimeInSamples - *timeInSamples;
        if (backwardDeltaSamples >= kBackwardJumpRetriggerMinSamples)
        {
...
{
    auto resetRuntimeStateForLoad = [this]()
    {
        // Runtime-only restart anchor is intentionally not persisted.
        // Reset on load to avoid stale sync points leaking across sessions/presets.
        ...
        // Runtime host transport snapshot is also intentionally non-persistent.
        ...
        // Loading persisted state should not emit deferred notifications from
        // pre-load runtime activity or recalculation side effects.
    ...
    {
        // Malformed/legacy inputs should still clear runtime-only timing residue.
        resetRuntimeStateForLoad();
        return;
    }

    auto sanitizeTimingMode = [](int value) {
    ...
    auto sanitizeNoteInterval = [](int value) {
        const int minValue = static_cast<int>(EngineNoteInterval::NOTE_1_32ND);
        const int maxValue = static_cast<int>(EngineNoteInterval::NOTE_TRIPLET_1_8TH);
        if (value >= minValue && value <= maxValue)
            return value;
        return static_cast<int>(EngineNoteInterval::NOTE_1_4TH);
    };

    auto sanitizeTriggerMode = [](int value) {
        const int minValue = static_cast<int>(WaveformTriggerMode::None);
        const int maxValue = static_cast<int>(WaveformTriggerMode::BothEdges);
        if (value >= minValue && value <= maxValue)
            return value;
        return static_cast<int>(WaveformTriggerMode::None);
    };
    ...
    if (!std::isfinite(timeInterval))
        timeInterval = EngineTimingConfig::DEFAULT_TIME_INTERVAL_MS;
    ...
    atomicNoteInterval_.store(
        sanitizeNoteInterval(static_cast<int>(state.getProperty(TimingIds::NoteInterval,
                                                                static_cast<int>(EngineNoteInterval::NOTE_1_4TH)))),
        std::memory_order_relaxed);
    atomicTriggerMode_.store(
        sanitizeTriggerMode(static_cast<int>(state.getProperty(TimingIds::TriggerMode,
                                                               static_cast<int>(WaveformTriggerMode::None)))),
        std::memory_order_relaxed);

    float threshold = static_cast<float>(state.getProperty(TimingIds::TriggerThreshold, 0.1f));
    if (!std::isfinite(threshold))
        threshold = 0.1f;
    threshold = juce::jlimit(0.0f, 1.0f, threshold);
    atomicTriggerThreshold_.store(threshold, std::memory_order_relaxed);

    int midiNote = static_cast<int>(state.getProperty(TimingIds::MidiTriggerNote, -1));
    atomicMidiTriggerNote_.store(juce::jlimit(-1, 127, midiNote), std::memory_order_relaxed);

    int midiChannel = static_cast<int>(state.getProperty(TimingIds::MidiTriggerChannel, 0));
    atomicMidiTriggerChannel_.store(juce::jlimit(0, 16, midiChannel), std::memory_order_relaxed);

    float internalBpm = static_cast<float>(state.getProperty(TimingIds::InternalBPM, 120.0f));
    if (!std::isfinite(internalBpm))
        internalBpm = 120.0f;
    internalBpm = juce::jlimit(EngineTimingConfig::MIN_BPM, EngineTimingConfig::MAX_BPM, internalBpm);
    atomicInternalBPM_.store(internalBpm, std::memory_order_relaxed);

    int triggerChannel = static_cast<int>(state.getProperty(TimingIds::TriggerChannel, 0));
    atomicTriggerChannel_.store(juce::jmax(0, triggerChannel), std::memory_order_relaxed);

    float triggerHysteresis = static_cast<float>(state.getProperty(TimingIds::TriggerHysteresis, 0.01f));
    if (!std::isfinite(triggerHysteresis))
        triggerHysteresis = 0.01f;
    triggerHysteresis = juce::jlimit(0.0f, 1.0f, triggerHysteresis);
    atomicTriggerHysteresis_.store(triggerHysteresis, std::memory_order_relaxed);
