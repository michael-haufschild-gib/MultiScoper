...
{
public:
    explicit MultiScoperModal(IThemeService& themeService);
    MultiScoperModal(IThemeService& themeService, const juce::String& title);
    MultiScoperModal(IThemeService& themeService, const juce::String& title, const juce::String& testId);
    ~MultiScoperModal() override;
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(MultiScoperModal)
};

/**
 * Helper class to show simple alert/confirm dialogs
 */
class MultiScoperAlertModal
{
public:
