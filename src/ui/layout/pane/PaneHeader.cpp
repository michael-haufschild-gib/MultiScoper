...
PaneHeader::PaneHeader(IThemeService& themeService)
...
void PaneHeader::resized()
{
    ...
    actionBar_->setBounds(
        getWidth() - CLOSE_BUTTON_SIZE - 2 - PADDING - actionBarWidth,
        0,
        actionBarWidth,
        HEIGHT
    );
    ...
    int labelWidth = juce::jmin(NAME_LABEL_WIDTH,
        getWidth() - labelX - actionBarWidth - CLOSE_BUTTON_SIZE - PADDING * 3);
    labelWidth = juce::jmax(0, labelWidth);  // Ensure non-negative
    nameLabel_->setBounds(
        labelX,
        (HEIGHT - nameLabel_->getPreferredHeight()) / 2,
        labelWidth,
...
juce::String PaneHeader::getPaneName() const
{
    return nameLabel_ ? nameLabel_->getText() : juce::String();
}
