...
- visual preset "CyberPunk Edge": see a screenshot: @screenshots/cyberpunk-edge-visual-preset.jpg   you see that the particles are forming two lines on the left half of the pane instead of following the waveform. and the waveform itself is just solid white - instead of showing a glowing neon cyberpunk look. nothing cyperpunk here.
-
- visual preset "Deep Ocean": the waveforms are not horizontally using the full available width of the pane they are rendered to. see screenshot: screenshots/_deep_ocean_visual_preset.jpg
- visual preset "Liquid Gold": the waveforms are not horizontally using the full available width of the pane they are rendered to. see screenshot: screenshots/_liquid_gold_visual_preset.jpg
- visual preset "Liquid Gold": same issue as "Deep Ocean" - the waveform fills maybe 1/3 of the screen
- visual preset "Synthwave Grid": nothing is rendered at all


do a full root cause analysis and fix the following issues with the "Cyberpunk Edge" visual preset:
1. the waveform is supposed to give off an 80s neon cyberpunk glow, but fails to do so. it is possible that this is only a configuration issue of the post-processing effects, or post-processing effects are buggy or wrongly implemented. this needs a careful a-to-z review.
2. there are particle effects that are misplaced. they should follow the whole waveform, instead there seems to be 1 emitter per channel which is just emitting particles in a horizontal line. it is possible that this is just a bad design decision, or that there is something fundamentally wrong with the particle system. the particles here should follow the shape of the waveform(s) across the whole visible length of the waveform(s).

see screenshots:
- when waveforms are screenshots/_cyberpunk_edge_running.jpg
- using hold: screenshots/_cyberpunk_edge_hold.jpg (you see the emitter that seems to be moving from left to right across several frames to create the horizontal particle lines you can see in the previous screenshot)


the shader looks very basic, a thin line with a bit of glow. the glow looks solid, not at all like actual light glow, like it has no opacity (are you using correct blend modes in the bloom?)

for inspiration a neon animation and shader closer to what we want:
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
 ...
 volumetric ligth shader from shadertoyes that is much closer to what we want than what you have created:
uniform vec3      iResolution;           // viewport resolution (in pixels)
uniform float     iTime;                 // shader playback time (in seconds)
uniform float     iTimeDelta;            // render time (in seconds)
uniform float     iFrameRate;            // shader frame rate
