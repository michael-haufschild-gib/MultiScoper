...
public:
     ...
     */
    static int getAnimationDuration(int normalDuration) { return prefersReducedMotion() ? 0 : normalDuration; }
     ...
     */
    static bool shouldUseSpringAnimations() { return !prefersReducedMotion(); }
     ...
     */
    static float getAnimationIntensity() { return prefersReducedMotion() ? 0.0f : 1.0f; }
     ...
     */
    static void clearOverride() { overrideReducedMotion_.store(false); }

    /**
     * Set app-level preference (for platforms without system support)
     */
    static void setAppPreference(bool reduceMotion) { appPrefersReducedMotion_.store(reduceMotion); }
...
{
public:
    ScopedReducedMotion() { AnimationSettings::setOverride(true, true); }

    ~ScopedReducedMotion() { AnimationSettings::clearOverride(); }
