...
## Decision
Graveyard pattern. It is simple, correct, and requires no special library support. The SpinLock on `bufferSwapLock_` serializes pointer reads (UI/render thread) and swaps (message thread). The audio thread uses `tryLock` and skips the write if contended (dropping frames rather than blocking).
