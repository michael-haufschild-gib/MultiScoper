/*
    MultiScoper - MultiScoperState Unit Tests
    ...
    - fromXmlString succeeding on non-MultiScoperState root element
*/
...
class MultiScoperStateTest : public ::testing::Test
{
protected:
...
TEST_F(MultiScoperStateTest, DefaultStateHasCurrentSchemaVersion)
{
    EXPECT_EQ(state->getSchemaVersion(), MultiScoperState::CURRENT_SCHEMA_VERSION);
}

TEST_F(MultiScoperStateTest, DefaultStateHasNoOscillators) { EXPECT_EQ(state->getOscillators().size(), 0); }

TEST_F(MultiScoperStateTest, DefaultStateHasNoPanes) { EXPECT_EQ(state->getLayoutManager().getPaneCount(), 0); }

TEST_F(MultiScoperStateTest, DefaultThemeIsDarkProfessional) { EXPECT_EQ(state->getThemeName(), "Dark Professional"); }

TEST_F(MultiScoperStateTest, DefaultColumnLayoutIsSingle) { EXPECT_EQ(state->getColumnLayout(), ColumnLayout::Single); }

TEST_F(MultiScoperStateTest, DefaultSidebarWidth) { EXPECT_EQ(state->getSidebarWidth(), 300); }

TEST_F(MultiScoperStateTest, DefaultSidebarNotCollapsed) { EXPECT_FALSE(state->isSidebarCollapsed()); }

TEST_F(MultiScoperStateTest, DefaultStatusBarVisible) { EXPECT_TRUE(state->isStatusBarVisible()); }

TEST_F(MultiScoperStateTest, DefaultShowGridEnabled) { EXPECT_TRUE(state->isShowGridEnabled()); }

TEST_F(MultiScoperStateTest, DefaultAutoScaleEnabled) { EXPECT_TRUE(state->isAutoScaleEnabled()); }

TEST_F(MultiScoperStateTest, DefaultGainDbIsZero) { EXPECT_FLOAT_EQ(state->getGainDb(), 0.0f); }

TEST_F(MultiScoperStateTest, DefaultCaptureQualityIsStandard)
{
...
TEST_F(MultiScoperStateTest, AddOscillatorIncreasesCount)
{
    addOscillator("Osc 1");
...
TEST_F(MultiScoperStateTest, GetOscillatorByIdReturnsCorrectOscillator)
{
...
TEST_F(MultiScoperStateTest, GetOscillatorWithInvalidIdReturnsNullopt)
{
...
TEST_F(MultiScoperStateTest, RemoveOscillatorDecreasesCount)
{
...
TEST_F(MultiScoperStateTest, RemoveOscillatorRenumbersOrderIndices)
{
...
TEST_F(MultiScoperStateTest, RemoveNonexistentOscillatorIsNoOp)
{
    addOscillator("Existing");
...
TEST_F(MultiScoperStateTest, UpdateOscillatorPersistsChanges)
{
...
TEST_F(MultiScoperStateTest, UpdateOscillatorPreservesVisualOverrides)
{
...
TEST_F(MultiScoperStateTest, ReorderOscillatorsSameIndexIsNoOp)
{
    addOscillator("A", 0);
    addOscillator("B", 1);
...
TEST_F(MultiScoperStateTest, ReorderOscillatorsOutOfBoundsIsNoOp)
{
    addOscillator("A", 0);
...
TEST_F(MultiScoperStateTest, ReorderOscillatorsMovesForward)
{
    addOscillator("A", 0);
    addOscillator("B", 1);
    addOscillator("C", 2);
...
TEST_F(MultiScoperStateTest, ReorderOscillatorsMovesBackward)
{
    addOscillator("A", 0);
    addOscillator("B", 1);
    addOscillator("C", 2);
...
TEST_F(MultiScoperStateTest, SetStatusBarVisiblePersists)
{
    state->setStatusBarVisible(false);
    EXPECT_FALSE(state->isStatusBarVisible());
}

TEST_F(MultiScoperStateTest, SetShowGridEnabledPersists)
{
    state->setShowGridEnabled(false);
    EXPECT_FALSE(state->isShowGridEnabled());
}

TEST_F(MultiScoperStateTest, SetAutoScaleEnabledPersists)
{
    state->setAutoScaleEnabled(false);
    EXPECT_FALSE(state->isAutoScaleEnabled());
}

TEST_F(MultiScoperStateTest, SetGainDbPreservesFloatPrecision)
{
...
TEST_F(MultiScoperStateTest, SetCaptureQualityConfigPersists)
{
...
TEST_F(MultiScoperStateTest, FromXmlStringRejectsEmptyString) { EXPECT_FALSE(state->fromXmlString("")); }

TEST_F(MultiScoperStateTest, FromXmlStringRejectsMalformedXml)
{
    EXPECT_FALSE(state->fromXmlString("<not <valid xml"));
}

TEST_F(MultiScoperStateTest, FromXmlStringRejectsWrongRootElement)
{
...
TEST_F(MultiScoperStateTest, ToXmlStringProducesNonEmptyOutput)
{
    juce::String xml = state->toXmlString();
    EXPECT_FALSE(xml.isEmpty());
}

TEST_F(MultiScoperStateTest, ValueTreeListenerReceivesPropertyChanges)
{
...
TEST_F(MultiScoperStateTest, ConstructFromValidXmlRestoresState)
{
    ...
    MultiScoperState restored(xml);
    EXPECT_EQ(restored.getThemeName(), "Neon");
    EXPECT_FLOAT_EQ(restored.getGainDb(), -3.0f);
    EXPECT_FALSE(restored.isShowGridEnabled());
}

TEST_F(MultiScoperStateTest, ConstructFromInvalidXmlFallsBackToDefaults)
{
    MultiScoperState restored("garbage data");

    // Should have valid defaults
    EXPECT_EQ(restored.getSchemaVersion(), MultiScoperState::CURRENT_SCHEMA_VERSION);
    EXPECT_EQ(restored.getThemeName(), "Dark Professional");
}

TEST_F(MultiScoperStateTest, ConstructFromEmptyXmlFallsBackToDefaults)
{
