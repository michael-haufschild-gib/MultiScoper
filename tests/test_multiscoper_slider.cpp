/*
    MultiScoper - Slider Component Tests
    Tests for MultiScoperSlider behavioral correctness
...
class MultiScoperSliderTest : public ::testing::Test
{
protected:
    ...
    MultiScoperSlider& makeSlider(double min = 0.0, double max = 100.0)
    {
        slider_ = std::make_unique<MultiScoperSlider>(getThemeManager());
...
private:
    ...
    std::unique_ptr<MultiScoperSlider> slider_;
};
...
TEST_F(MultiScoperSliderTest, ValueIsClampedToRangeBounds)
{
...
TEST_F(MultiScoperSliderTest, ValueQuantizedToStepSize)
{
...
TEST_F(MultiScoperSliderTest, StepQuantizationWithNonZeroMinimum)
{
...
TEST_F(MultiScoperSliderTest, StepOfZeroDisablesQuantization)
{
...
TEST_F(MultiScoperSliderTest, ValueReconstrainedWhenRangeChanges)
{
...
TEST_F(MultiScoperSliderTest, SetValueWithNotifyTrueFiresCallback)
{
    auto& slider = makeSlider(0.0, 100.0);
    int callCount = 0;
...
TEST_F(MultiScoperSliderTest, SetValueWithNotifyFalseSuppressesCallback)
{
    auto& slider = makeSlider(0.0, 100.0);
    int callCount = 0;
...
TEST_F(MultiScoperSliderTest, SetValueToSameValueDoesNotFireCallback)
{
...
TEST_F(MultiScoperSliderTest, DeadZoneThresholdIsRespected)
{
...
TEST_F(MultiScoperSliderTest, RangeValuesAutoSwappedWhenInverted)
{
    // Bug: setting start > end silently corrupts range
    MultiScoperSlider slider(getThemeManager(), SliderVariant::Range);
...
TEST_F(MultiScoperSliderTest, RangeCallbackReceivesNormalizedValues)
{
    MultiScoperSlider slider(getThemeManager(), SliderVariant::Range);
    slider.setRange(0.0, 100.0);
...
TEST_F(MultiScoperSliderTest, RangeCallbackNotFiredWhenValuesUnchanged)
{
    MultiScoperSlider slider(getThemeManager(), SliderVariant::Range);
    slider.setRange(0.0, 100.0);
...
TEST_F(MultiScoperSliderTest, RangeValuesClampedToSliderRange)
{
    MultiScoperSlider slider(getThemeManager(), SliderVariant::Range);
...
TEST_F(MultiScoperSliderTest, VerticalVariantHasTallPreferredSize)
{
    MultiScoperSlider vertical(getThemeManager(), SliderVariant::Vertical);
    MultiScoperSlider horizontal(getThemeManager(), SliderVariant::Single);
...
TEST_F(MultiScoperSliderTest, DisabledSliderStillAcceptsValueChanges)
{
...
TEST_F(MultiScoperSliderTest, DefaultValueAddedAsMagneticPoint)
{
    auto& slider = makeSlider(0.0, 100.0);
...
TEST_F(MultiScoperSliderTest, MagneticSnappingCanBeToggledIndependentlyOfPoints)
{
    auto& slider = makeSlider(0.0, 100.0);
...
TEST_F(MultiScoperSliderTest, ThemeChangePreservesValueAndConfiguration)
{
    auto& slider = makeSlider(0.0, 100.0);
...
TEST_F(MultiScoperSliderTest, ZeroWidthRangeDoesNotCrash)
{
...
TEST_F(MultiScoperSliderTest, NegativeRange)
{
...
TEST_F(MultiScoperSliderTest, VerySmallStepDoesNotInfiniteLoop)
{
    auto& slider = makeSlider(0.0, 1.0);
...
TEST_F(MultiScoperSliderTest, SetRangeUpdatesDefaultMagneticPoints)
{
