---
description: Comprehensive code quality audit for vibecoding mistakes, hallucinations, and agent drift.
---

# Vibecoding & Code Quality Audit

**Purpose:** Catch LLM coding agent mistakes (hallucinations, code duplication, vibecoding) before they cause problems.

## Phase 0: Initialize Audit

1. Use mcp__mcp_docker__sequentialthinking to plan audit scope:
   - Which modules/domains to audit
   - Risk areas (recent changes, complex logic, API boundaries)
   - Specific concerns (if user mentioned any)

2. Use TodoWrite to track audit findings:
   - Will add findings as they're discovered
...
## Phase 1: Conduct Audit

Now produce the review using the OUTPUT FORMAT exactly.

## Phase 2: Track Findings in TodoWrite

**After completing the audit:**

1. Add each Critical/Major finding as a TodoWrite task:
   - Task description: "[Severity] [Rule] [File]: [Issue]"
   - Example: "Critical SEC-01 app/Http/Controllers/UserController.php:45: SQL injection via string concat"

2. Prioritize tasks:
   - Critical issues first
   - Major issues second
   - Moderate/Minor can be batched

## Phase 3: Actionable Summary

**Provide clear next steps:**

1. **Immediate actions** (Critical/Major findings):
   - List specific files and lines to fix
   - Provide exact fix recommendations
   - Estimated effort for each

2. **Follow-up actions** (Moderate/Minor):
   - Can be addressed in future work
   - Group related issues

3. **Documentation needs:**
   - If findings reveal missing docs → Note which canonical docs to update
   - NO creation of audit reports or summary documents

**Final Output Template:**
"Vibecheck complete. Found:
- X Critical issues (require immediate fix)
- Y Major issues (require attention)
- Z Moderate/Minor issues (future work)

All findings added to TodoWrite. See tasks for detailed fixes.
