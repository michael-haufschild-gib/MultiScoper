...
- Pure UI widgets live beside entry points (`src/ui/components/MultiScoperButton.cpp` vs. `src/ui/PluginEditor.cpp`) which makes it hard to isolate reusable UI code or bundle-only surfaces.
...
src/
  ...
  shared/
    core/                # MultiScoperState, InstanceRegistry, SharedCaptureBuffer
    dsp/                 # SignalProcessor, TimingEngine
    rendering/           # Engine, Framebuffer, ShaderRegistry, Effect base classes
    ui_kit/              # Generic widgets (MultiScoperButton, MultiScoperSlider, ColorPicker)
  modules/
...
3. **Phase 3 – Stabilize shared layers**: Relocate `InstanceRegistry.cpp`, `MultiScoperState.cpp`, `SharedCaptureBuffer.cpp`, `SignalProcessor.cpp`, `TimingEngine.cpp`, render primitives, and UI atoms into `src/shared/*`. Ensure headers under `include/` match the new paths.
