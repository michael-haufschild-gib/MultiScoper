...
{
    if (str == "HOST_SYNC")
        return TriggerMode::HOST_SYNC;
    if (str == "TRIGGERED")
        return TriggerMode::TRIGGERED;
    if (str == "MIDI")
        return TriggerMode::MIDI;
...
inline juce::String waveformModeToString(WaveformMode mode)
{
    switch (mode)
    {
        ...
        case WaveformMode::RestartOnPlay:
            return "Restart on Play";
        case WaveformMode::RestartOnNote:
            return "Restart on Note";
    }
    return "Free Running";
}
...
{
    if (str == "Restart on Play")
        return WaveformMode::RestartOnPlay;
    if (str == "Restart on Note")
        return WaveformMode::RestartOnNote;
