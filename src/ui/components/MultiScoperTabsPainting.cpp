/*
    MultiScoper - Tabs Component Painting
    Rendering, layout calculation, and indicator animation for MultiScoperTabs
*/
...
void MultiScoperTabs::paint(juce::Graphics& g)
{
    ...
    if (hasFocus_)
    {
        ...
        SurfacePainter::paintFocusRing(g, selectedBounds, 0.0f, getSurface().accent);
    }
}

void MultiScoperTabs::paintTab(juce::Graphics& g, int index, juce::Rectangle<int> bounds)
{
...
void MultiScoperTabs::paintTabContent(juce::Graphics& g, int index, juce::Rectangle<int> bounds, bool isSelected,
                                      bool isHovered)
{
    const auto& tab = tabs_[static_cast<size_t>(index)];
...
void MultiScoperTabs::paintIndicator(juce::Graphics& g)
{
    if (tabs_.empty())
        return;
    ...
    switch (variant_)
    {
        ...
        case Variant::Pills:
            ...
            g.drawRect(indicatorBounds.reduced(2).toFloat(), 1.0f);
            break;

        case Variant::Bordered:
            g.setColour(getSurface().borderDefault);
            g.drawRect(indicatorBounds.reduced(1).toFloat(), 1.0f);
            break;
    }
}

void MultiScoperTabs::paintBadge(juce::Graphics& g, juce::Rectangle<int> bounds, int count)
{
...
juce::Rectangle<int> MultiScoperTabs::getTabBounds(int index) const
{
...
void MultiScoperTabs::updateHorizontalLayoutCache(juce::Rectangle<int> bounds)
{
...
void MultiScoperTabs::updateVerticalLayoutCache(juce::Rectangle<int> bounds)
{
...
void MultiScoperTabs::updateLayoutCache()
{
...
juce::Rectangle<float> MultiScoperTabs::getIndicatorBounds() const
{
    if (orientation_ == Orientation::Horizontal)
    {
