...
void TestHttpServer::handleVerifyWaveform(const httplib::Request& req, httplib::Response& res)
{
    try
    {
        ...
        {
            res.set_content(errorResponse("Element not found").dump(), "application/json");
            return;
        }

        auto image = screenshot_.getComponentImage(component);
        bool hasContent = screenshot_.hasRenderedContent(image);
        float amplitude = screenshot_.estimateWaveformAmplitude(image);
        auto avgColor = screenshot_.getAverageColor(image);

        json data;
        data["rendering"] = hasContent;
        data["amplitude"] = amplitude;
        data["avgColor"] = avgColor.toString().toStdString();
