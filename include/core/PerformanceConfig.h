...
struct PerformanceConfig
{
     ...
     */
    bool isCpuWarning(float cpuUsage) const { return cpuUsage >= cpuWarningThreshold; }

    /**
     * Check if CPU usage is critical
     */
    bool isCpuCritical(float cpuUsage) const { return cpuUsage >= cpuCriticalThreshold; }

    /**
     * Check if memory usage is above warning threshold
     */
    bool isMemoryWarning(int memoryMb) const { return memoryMb >= memoryWarningThreshold; }

    /**
     * Check if memory usage is critical
     */
    bool isMemoryCritical(int memoryMb) const { return memoryMb >= memoryCriticalThreshold; }
        ...
        if (cpuUsage >= cpuWarningThreshold)
        {
            ...
            switch (qualityMode)
            {
                ...
                case QualityMode::PERFORMANCE:
                    return QualityMode::MAXIMUM;
                case QualityMode::MAXIMUM:
                    return QualityMode::MAXIMUM; // Already at lowest
            }
        }
        return qualityMode;
    }
    ...
    void fromValueTree(const juce::ValueTree& tree)
    {
        ...
        decimationLevel =
            juce::jlimit(MIN_DECIMATION, MAX_DECIMATION, static_cast<int>(tree.getProperty("decimationLevel", 0)));
...
struct StatusBarConfig
{
    ...
    void fromValueTree(const juce::ValueTree& tree)
    {
        ...
        position = tree.getProperty("position", "BOTTOM").toString() == "TOP" ? StatusBarPosition::TOP
