/*
    MultiScoper - Meter Bar Component Implementation
*/
...
MultiScoperMeterBar::MultiScoperMeterBar(IThemeService& themeService) : ThemedComponent(themeService)
{
    startTimerHz(TIMER_HZ);
}

MultiScoperMeterBar::MultiScoperMeterBar(IThemeService& themeService, const juce::String& testId)
    : MultiScoperMeterBar(themeService)
{
    setTestId(testId);
}

void MultiScoperMeterBar::registerTestId() { MULTISCOPER_REGISTER_TEST_ID(testId_); }

MultiScoperMeterBar::~MultiScoperMeterBar() { stopTimer(); }

void MultiScoperMeterBar::setLevel(float level)
{
    leftLevel_ = level;
    rightLevel_ = level;
...
void MultiScoperMeterBar::setLevels(float left, float right)
{
    leftLevel_ = left;
    rightLevel_ = right;
...
void MultiScoperMeterBar::setRMSLevel(float rms) { leftRMS_ = rightRMS_ = rms; }

void MultiScoperMeterBar::setRMSLevels(float leftRMS, float rightRMS)
{
    leftRMS_ = leftRMS;
    rightRMS_ = rightRMS;
}

void MultiScoperMeterBar::setOrientation(Orientation orientation)
{
...
void MultiScoperMeterBar::setMeterType(MeterType type)
{
    if (meterType_ != type)
    {
        meterType_ = type;
...
void MultiScoperMeterBar::setStereo(bool stereo)
{
    if (stereo_ != stereo)
    {
        stereo_ = stereo;
...
void MultiScoperMeterBar::setPeakHoldTime(int milliseconds) { peakHoldTimeMs_ = milliseconds; }

void MultiScoperMeterBar::setPeakDecayRate(float decayPerSecond) { peakDecayRate_ = decayPerSecond; }

void MultiScoperMeterBar::setMinDb(float db) { minDb_ = db; }

void MultiScoperMeterBar::setMaxDb(float db) { maxDb_ = db; }

void MultiScoperMeterBar::setShowScale(bool show)
{
    if (showScale_ != show)
    {
        showScale_ = show;
...
void MultiScoperMeterBar::resetClip()
{
    leftClip_ = rightClip_ = false;
    repaint();
}

void MultiScoperMeterBar::resetPeakHold()
{
    leftPeakHold_ = rightPeakHold_ = 0.0f;
    leftPeakHoldCounter_ = rightPeakHoldCounter_ = 0;
}

int MultiScoperMeterBar::getPreferredWidth() const
{
    int const meterWidth = stereo_ ? STEREO_WIDTH : MONO_WIDTH;
    return meterWidth + (showScale_ ? SCALE_WIDTH : 0);
}

int MultiScoperMeterBar::getPreferredHeight() const { return 100; }

void MultiScoperMeterBar::paint(juce::Graphics& g)
{
...
void MultiScoperMeterBar::paintMeterVertical(juce::Graphics& g, const juce::Rectangle<int>& bounds,
                                             juce::ColourGradient& gradient, float levelPos, float rmsPos,
                                             float peakPos, bool clip)
{
...
void MultiScoperMeterBar::paintMeterHorizontal(juce::Graphics& g, const juce::Rectangle<int>& bounds,
                                               juce::ColourGradient& gradient, float levelPos, float rmsPos,
                                               float peakPos, bool clip)
{
...
void MultiScoperMeterBar::paintMeter(juce::Graphics& g, juce::Rectangle<int> bounds, float level, float rms,
                                     float peakHold, bool clip)
{
...
void MultiScoperMeterBar::paintScale(juce::Graphics& g, juce::Rectangle<int> bounds)
{
...
float MultiScoperMeterBar::levelToPosition(float level) const
{
    if (level <= 0.0f)
        return 0.0f;
...
float MultiScoperMeterBar::dbToLevel(float db) const { return std::pow(10.0f, db / 20.0f); }

void MultiScoperMeterBar::resized()
{
    // Nothing to do
}

void MultiScoperMeterBar::mouseDown(const juce::MouseEvent& /*event*/)
{
    // Click to reset clip
    resetClip();
}

void MultiScoperMeterBar::timerCallback()
{
