...
namespace StateIds
{
inline const juce::Identifier MultiScoperState{"MultiScoperState"};
...
inline const juce::Identifier TrackIdentifier{"trackIdentifier"};
...
/**
 * Central state manager for MultiScoper plugin.
 * Handles serialization, deserialization, and migration of plugin state.
 */
class MultiScoperState
{
public:
    /// Create a default MultiScoperState with initialized state tree.
    MultiScoperState();

    /**
     * Create state from XML string (from DAW)
     */
    explicit MultiScoperState(const juce::String& xmlString);
    ...
    /**
     * Get/set the persistent track identifier for this plugin instance.
     * Empty when the state originated from a pre-v3 save (legacy saves have no
     * persistent identity — callers must fall back to a fresh UUID).
     */
    [[nodiscard]] juce::String getTrackIdentifier() const;
    void setTrackIdentifier(const juce::String& trackIdentifier);
