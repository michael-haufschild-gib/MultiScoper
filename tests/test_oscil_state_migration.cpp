/*
    Oscil - OscilState Fixture-Driven Migration Tests
...
class OscilStateMigrationFixturesTest : public ::testing::Test
{
protected:
    oscil::OscilState state;
};

TEST_F(OscilStateMigrationFixturesTest, V0FixtureLoadsAndIsStampedCurrent)
{
    const auto xml = loadFixtureText("oscil_state_v0.xml");
    ASSERT_FALSE(xml.isEmpty()) << "v0 fixture not found on disk";

    EXPECT_TRUE(state.fromXmlString(xml));
    EXPECT_EQ(state.getSchemaVersion(), oscil::OscilState::CURRENT_SCHEMA_VERSION);
...
TEST_F(OscilStateMigrationFixturesTest, V1FixtureLoadsAndIsStampedCurrent)
{
    const auto xml = loadFixtureText("oscil_state_v1.xml");
...
TEST_F(OscilStateMigrationFixturesTest, FutureFixtureIsRejectedAndPreservesState)
{
    const auto xml = loadFixtureText("oscil_state_v_future.xml");
