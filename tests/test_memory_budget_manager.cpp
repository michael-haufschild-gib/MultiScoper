...
TEST_F(MemoryBudgetQualityOverrideTest, UseGlobalOverrideUsesRecommendedWhenAutoAdjustEnabled)
{
    CaptureQualityConfig config;
    config.qualityPreset = QualityPreset::High;
    config.autoAdjustQuality = true;
...
TEST_F(MemoryBudgetPruneTest, ExpiredBuffersArePrunedAutomaticallyByOperations)
{
    {
        CaptureQualityConfig config;
        auto buffer = std::make_shared<DecimatingCaptureBuffer>(config, 44100);
